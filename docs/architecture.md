...
La API **no lanza agentes ni escribe en el workspace**, ni sirve el libro de regalo: el PDF lo genera `novela exportar --formato pdf` y se entrega como fichero (ADR 0003). No hay verbo de escritura: mutar una novela es trabajo del orquestador a través del CLI. Si un endpoint pareciera necesitar escribir, lo correcto es añadir un subcomando al CLI, no un `POST` a la API.
