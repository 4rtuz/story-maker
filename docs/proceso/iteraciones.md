...
| 2026-09-25 | Incidente (`ejemplo-carmen`, cap. 5) | El escritor se negó a escribir: la ficha 05 contradecía el canon (una pista incompatible con el horario de Tango, horas imposibles y «guardó la carta setenta años» con una protagonista de 63). Raíz: la ficha de Tango la añadió el arquitecto sin conocer el misterio | El trazador rehízo solo la ficha 05 con un briefing nuevo; `validar-plan` pasa. Lección: un parche de canon autorizado a posteriori necesita al trazador, que sí ve el misterio | — |
| 2026-09-25 | Incidente (`ejemplo-carmen`, auditoría) | `verificar-lean` pasaba a `lean` una ruta relativa con otro cwd (ENOENT) | Ruta absoluta; en la novela real, 32 eventos y 4/4 invariantes | `bcc2961` |
| 2026-09-25 | Incidente (`ejemplo-carmen`, export) | Lugares con ids inventados en los capítulos 1–2 (cronista v1) dejaban la novela sin PDF | `aplicar-delta` rechaza lugares sin ficha; la ficha del libro muestra el lugar con el nombre de su id | `5ab7151` |
| 2026-09-25 | Incidente (`ejemplo-carmen` v2, cap. 1) | El `lector-suspense` (haiku) escribió `qa/01-suspense.json` fuera de esquema (campos de más, veredicto en mayúsculas); el orquestador lo leyó como aprobado y `aplicar-delta` salió con 4 | Reintento manual del revisor. Pendiente: validar los informes de revisión contra su esquema en el CLI antes del gate de revisión, con reintento del revisor y no intervención | — |
