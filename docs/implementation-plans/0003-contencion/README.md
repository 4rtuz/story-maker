...
Cómo se construye `.claude/` y cómo se cierran los tres cambios de backend que pide la spec
...
| 3 | [fase-3-procedencia.md](fase-3-procedencia.md) | `fase`, `sucio`, `hashes_claude` y `sesion=` (TDD) | El capítulo 1 se atribuye a su canon real |
...
- El backend de la 0001 está entero en esta rama (`spec-0003`, que sale de `spec-backend`). Los
  ficheros que se tocan son cuatro: `novela/dominio/artefactos.py` (`Manifest`),
  `novela/plataforma/run.py`, `novela/slices/briefing/cmd.py` y `api/openapi.json`, que se
  regenera.
...
| RF-05 | Hook: nada bajo `estado/` salvo `deltas/NN.json`, normalizado | 2 | 2.2 |
...
| RF-12 | `sucio` y `hashes_claude` en el manifiesto | 3 | 3.2 |
...
| RF-18 | Canario: cuatro intentos, cuatro fallos en disco | 5 | 5.2, 5.3 |
| RF-19 | Novela de humo con `checkpoints/03.json` y baseline | 7 | 7.2, 7.4 |
| RF-20 | Hook: rama `Bash` | 2 | 2.4 |
| RF-21 | `sesion=<uuid>` en `harness.log` | 3 | 3.3 |
| RF-22 | El bucle para sin avance de checkpoint | 6 | 6.2, 6.3 |
| RF-23 | `AGENTS.md` documenta la puesta en marcha | 6 | 6.2 |

Los 13 criterios de aceptación van nombrados en la tarea que los cierra.
...
La spec no dice nada sobre estos puntos y hay que teclear algo. Todas son baratas de revertir.

| # | Decisión | Motivo | Tarea |
|---|---|---|---|
| D-1 | El cuerpo de cada agente nombra la ruta de su esquema de salida en `backend/schemas/` | El briefing no incrusta esquemas (`assemble.py` no los toca), y la salida estructurada se exige «JSON válido contra su esquema». Leer dentro del proyecto no pide permiso | 1.2 |
...
| D-5 | `sesion=<uuid>` va justo después de la marca de tiempo | No choca con las causas, que van al final tras `·`, y no altera las subcadenas `validar NN -> 1` que cuenta el procedimiento | 3.3 |
| D-6 | Sin git disponible, `sucio` es `true` | Conservador: un manifiesto que no puede probar que el árbol estaba limpio no lo afirma | 3.2 |
...
| D-9 | Un `intervencion.md` está resuelto cuando tiene una línea `resuelto: <sha o motivo>` | La spec manda parar ante uno «sin resolver» y no dice cómo se resuelve. Borrarlo perdería el registro | 4.3, 7.2 |
...
1. **El hook falla abierto si `python` no resuelve.** Claude Code trata un código distinto de 2
   ...
   Microsoft Store (E-11). El hook se registra con `python`, y la fase 6 comprueba que resuelve.
   El canario (fase 5) lo detecta: la escritura de `estado.db` pasaría.
...
4. **`NOVELA_RUN_ID` fijado se salta `fase`.** Con la variable definida, arranque y capítulo 1
   comparten run. El bucle no la exporta. Se deja anotado en el código (tarea 3.1).
